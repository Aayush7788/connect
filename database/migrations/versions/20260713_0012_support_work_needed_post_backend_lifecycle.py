"""support work needed post backend lifecycle

Revision ID: 20260713_0012
Revises: 20260713_0011
Create Date: 2026-07-13 18:30:00.000000
"""

from collections.abc import Sequence

from alembic import op
import sqlalchemy as sa

revision: str = "20260713_0012"
down_revision: str | None = "20260713_0011"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None


def upgrade() -> None:
    op.add_column(
        "work_needed_posts",
        sa.Column("creation_idempotency_key", sa.Text(), nullable=True),
    )
    op.add_column(
        "work_needed_posts",
        sa.Column("creation_request_hash", sa.Text(), nullable=True),
    )
    op.create_check_constraint(
        op.f("ck_work_needed_posts_idempotency_fields_together"),
        "work_needed_posts",
        "(creation_idempotency_key is null and creation_request_hash is null) or "
        "(creation_idempotency_key is not null and creation_request_hash is not null)",
    )
    op.create_check_constraint(
        op.f("ck_work_needed_posts_idempotency_key_length"),
        "work_needed_posts",
        "creation_idempotency_key is null or "
        "char_length(creation_idempotency_key) between 1 and 128",
    )
    op.create_index(
        "uq_work_needed_posts_creation_idempotency",
        "work_needed_posts",
        ["profile_id", "creation_idempotency_key"],
        unique=True,
        postgresql_where=sa.text("creation_idempotency_key is not null"),
    )
    op.drop_constraint(
        op.f("ck_work_needed_posts_publish_requires_required_fields"),
        "work_needed_posts",
        type_="check",
    )
    op.execute(
        sa.text(
            "update work_needed_posts "
            "set status = 'draft', closed_at = null "
            "where status in ('active', 'paused', 'closed_by_user') "
            "and nullif(btrim(description), '') is null"
        )
    )
    op.create_check_constraint(
        op.f("ck_work_needed_posts_publish_requires_required_fields"),
        "work_needed_posts",
        "status not in ('active', 'paused', 'closed_by_user') or "
        "(photo_count >= 3 and "
        "nullif(btrim(description), '') is not null and "
        "(work_category_id is not null or "
        "nullif(btrim(custom_work_category_text), '') is not null) and "
        "(work_name_category_id is not null or "
        "nullif(btrim(custom_work_name), '') is not null))",
    )


def downgrade() -> None:
    op.drop_constraint(
        op.f("ck_work_needed_posts_publish_requires_required_fields"),
        "work_needed_posts",
        type_="check",
    )
    op.create_check_constraint(
        op.f("ck_work_needed_posts_publish_requires_required_fields"),
        "work_needed_posts",
        "status not in ('active', 'paused', 'closed_by_user') or "
        "(photo_count >= 3 and "
        "(work_category_id is not null or "
        "nullif(btrim(custom_work_category_text), '') is not null) and "
        "(work_name_category_id is not null or "
        "nullif(btrim(custom_work_name), '') is not null))",
    )
    op.drop_index(
        "uq_work_needed_posts_creation_idempotency",
        table_name="work_needed_posts",
        postgresql_where=sa.text("creation_idempotency_key is not null"),
    )
    op.drop_constraint(
        op.f("ck_work_needed_posts_idempotency_key_length"),
        "work_needed_posts",
        type_="check",
    )
    op.drop_constraint(
        op.f("ck_work_needed_posts_idempotency_fields_together"),
        "work_needed_posts",
        type_="check",
    )
    op.drop_column("work_needed_posts", "creation_request_hash")
    op.drop_column("work_needed_posts", "creation_idempotency_key")
