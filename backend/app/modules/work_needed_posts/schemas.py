...
class WorkNeededPostUpsertRequest(BaseModel):
    ...
    def validate_mapped_or_custom_fields(self) -> "WorkNeededPostUpsertRequest":
...
class WorkNeededPostResponse(BaseModel):
    id: UUID
    profile_id: UUID
    status: WorkNeededPostStatus
    title: str
    category_id: UUID | None = None
...
class WorkNeededPostListResponse(BaseModel):
    items: list[WorkNeededPostResponse] = Field(default_factory=list)
